package com.piedraverde.myapplication;

import android.content.ContentValues;
import android.content.Context;
import android.database.Cursor;
import android.database.sqlite.SQLiteDatabase;
import android.database.sqlite.SQLiteOpenHelper;

import java.util.ArrayList;
import java.util.HashMap;

public class DbHelper extends SQLiteOpenHelper {

    public static final String TBL_USERS = "users",
            TBL_USERS_ID = "id",
            TBL_USERS_USERNAME = "username",
            TBL_USERS_PASSWORD = "password",
            TBL_USERS_NAME = "name",
            TBL_USERS_GENDER = "gender";

    SQLiteDatabase dbReadable = getReadableDatabase();
    SQLiteDatabase dbWritable = getWritableDatabase();


    public DbHelper(Context context) {
        super(context, "db_databasing",  null, 1);
    }

    @Override
    public void onCreate(SQLiteDatabase db) {
        String sql_create_users_table = String.format("CREATE TABLE %s " +
                "(%s INTEGER PRIMARY KEY AUTOINCREMENT, " +
                "%s TEXT," +
                "%s TEXT," +
                "%s TEXT," +
                "%s TEXT)",
                TBL_USERS,
                TBL_USERS_ID,
                TBL_USERS_USERNAME ,
                TBL_USERS_PASSWORD ,
                TBL_USERS_NAME ,
                TBL_USERS_GENDER);

        db.execSQL(sql_create_users_table);


    }

    @Override
    public void onUpgrade(SQLiteDatabase db, int oldVersion, int newVersion) {

    }

    public int addUser(HashMap<String,String> map_user) {

        ContentValues val = new ContentValues();

        String sql = "SELECT * FROM " + TBL_USERS +
                " WHERE " + TBL_USERS_NAME + " = '" + map_user.get(TBL_USERS_USERNAME) + "'";
        Cursor cur = dbReadable.rawQuery(sql, null);
        int userID = 0;

        if (cur.moveToNext()){
            userID = cur.getInt(cur.getColumnIndex(TBL_USERS_ID));
        }
        else {
            val.put(TBL_USERS_USERNAME, map_user.get(TBL_USERS_USERNAME));
            val.put(TBL_USERS_PASSWORD, map_user.get(TBL_USERS_PASSWORD));
            val.put(TBL_USERS_NAME, map_user.get(TBL_USERS_NAME));
            val.put(TBL_USERS_GENDER, map_user.get(TBL_USERS_GENDER));

            dbWritable.insert(TBL_USERS, null, val);
        }


        return userID;
    }

    public int checkUser(HashMap<String,String> map_user) {

        String sql = " SELECT  * FROM " + TBL_USERS +
                " WHERE " + TBL_USERS_NAME + " ='" + map_user.get(TBL_USERS_USERNAME) + "' AND " +
                TBL_USERS_PASSWORD +  " = '" + map_user.get(TBL_USERS_PASSWORD) + "'";
        Cursor cur  = dbReadable.rawQuery(sql, null);
        int userId = 0;

        if (cur.moveToNext()){
            userId = cur.getInt(cur.getColumnIndex(TBL_USERS_ID));
        }

        return userId;
    }

    public ArrayList<HashMap<String,String>> getAllUser() {
        String sql = " SELECT * FROM " + TBL_USERS + " ORDER BY " + TBL_USERS_NAME + " ASC ";
        Cursor cur = dbReadable.rawQuery(sql, null);

        ArrayList<HashMap<String, String>> all_users = new ArrayList();

        while(cur.moveToNext()){
            HashMap<String, String> map_user = new HashMap();
            map_user.put(TBL_USERS_ID, cur.getString(cur.getColumnIndex(TBL_USERS_ID)));
            map_user.put(TBL_USERS_USERNAME, cur.getString(cur.getColumnIndex(TBL_USERS_USERNAME)));
            map_user.put(TBL_USERS_PASSWORD, cur.getString(cur.getColumnIndex(TBL_USERS_PASSWORD)));
            map_user.put(TBL_USERS_NAME, cur.getString(cur.getColumnIndex(TBL_USERS_NAME)));
            map_user.put(TBL_USERS_GENDER, cur.getString(cur.getColumnIndex(TBL_USERS_GENDER)));
            all_users.add(map_user);

        }
        cur.close();


        return all_users;
    }

    public void deleteUser(int userID) {
        dbWritable.delete(TBL_USERS, TBL_USERS_ID + " = " + userID, null);
    }

    public ArrayList<HashMap<String,String>> getSelectedUserData(int userID) {

        String sql = " SELECT * FROM " + TBL_USERS + " WHERE " + TBL_USERS_ID  + " = " + userID;
        Cursor cur = dbReadable.rawQuery(sql, null);

        ArrayList<HashMap<String, String>> selected_user = new ArrayList();

        while(cur.moveToNext()){
            HashMap<String, String> map_user = new HashMap();
            map_user.put(TBL_USERS_USERNAME, cur.getString((cur.getColumnIndex(TBL_USERS_USERNAME))));
            map_user.put(TBL_USERS_PASSWORD, cur.getString((cur.getColumnIndex(TBL_USERS_PASSWORD))));
            map_user.put(TBL_USERS_GENDER, cur.getString((cur.getColumnIndex(TBL_USERS_GENDER))));
            map_user.put(TBL_USERS_NAME, cur.getString((cur.getColumnIndex(TBL_USERS_NAME))));
            selected_user.add(map_user);
        }

        cur.close();

        return selected_user;
    }

    public void updateUser(HashMap<String,String>  map_user) {
        ContentValues val = new ContentValues();
        val.put(TBL_USERS_USERNAME, map_user.get(TBL_USERS_USERNAME));
        val.put(TBL_USERS_PASSWORD, map_user.get(TBL_USERS_PASSWORD));
        val.put(TBL_USERS_NAME, map_user.get(TBL_USERS_NAME));
        val.put(TBL_USERS_GENDER, map_user.get(TBL_USERS_GENDER));

        dbWritable.update(TBL_USERS , val , TBL_USERS_ID + " = " + map_user.get(TBL_USERS_ID), null);
    }
}
